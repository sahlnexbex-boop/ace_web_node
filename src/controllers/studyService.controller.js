...
export const createStudyService = async (req, res) => {
  try {
    ...
    if (![1, 2, 3, 4, 5].includes(Number(service_type)))
      return res
        .status(400)
        .json({ message: "Invalid service_type. Only 1-5 allowed." });
    ...
    res
      .status(201)
      .json({
        message: "Study Service created successfully",
        data: newService,
      });
  } catch (err) {
...
export const getStudyServices = async (req, res) => {
  try {
    ...
    if (service_type && [1, 2, 3, 4, 5].includes(Number(service_type)))
      where.service_type = Number(service_type);
...
export const updateStudyService = async (req, res) => {
  try {
    ...
    if(newFile && isUnsupportedFile(req.file.mimetype)) {
      return res.status(400).json({ message: "Invalid file type. images & video not allowed" });
    }
