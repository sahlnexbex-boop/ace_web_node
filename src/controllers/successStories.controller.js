...
export const createSuccessStory = async (req, res) => {
  try {
    const { stories_title, name_of_candidate, year, description, course_category_id, youtube_video_link, status } = req.body;
    const thumbnail_image = req.file ? `${SERVER_URL}/uploads/success_stories/${req.file.filename}` : null;
    ...
    if (course_category_id) {
      ...
      if (!categoryExists) return res.status(400).json({ message: "Invalid course_category_id" });
    }
...
export const getSuccessStories = async (req, res) => {
  try {
    const { page = 1, limit = 10, search = "", status, year } = req.query;
    const offset = (page - 1) * limit;

    const where = {};
    if (search) where.stories_title = { [Op.like]: `%${search}%` };
...
export const updateSuccessStory = async (req, res) => {
  try {
    const { id } = req.params;
    const { stories_title, name_of_candidate, year, description, course_category_id, youtube_video_link, status } = req.body;
    const newThumbnail = req.file ? `${SERVER_URL}/uploads/success_stories/${req.file.filename}` : null;
    ...
    if (newThumbnail && story.thumbnail_image) deleteFile(story.thumbnail_image);
    ...
    story.status = [0, 1].includes(Number(status)) ? Number(status) : story.status;
